<?php
/**
 * AddClassEnrollmentScheduleRequest
 *
 * PHP version 5
 *
 * @category Class
 * @package  Nlocascio\Mindbody
 * @author   Swagger Codegen team
 * @link     https://github.com/swagger-api/swagger-codegen
 */

/**
 * MINDBODY Public API
 *
 * No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)
 *
 * OpenAPI spec version: v6
 * 
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 */

/**
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen
 * Do not edit the class manually.
 */

namespace Nlocascio\Mindbody\Model;

/**
 * AddClassEnrollmentScheduleRequest Class Doc Comment
 *
 * @category Class
 * @package  Nlocascio\Mindbody
 * @author   Swagger Codegen team
 * @link     https://github.com/swagger-api/swagger-codegen
 * @property int $ClassDescriptionId The Id of the class/enrollment description. This can be found in GetClassDescriptions.
 * @property int $LocationId The Location Id of the enrollment schedule.
 * @property \DateTime $StartDate The start date of the enrollment schedule.
 * @property \DateTime $EndDate The end date of the enrollment schedule.
 * @property \DateTime $StartTime Enrollment start time (use null or omit for TBD).
 * @property \DateTime $EndTime Enrollment end time (ignored if StartTime is null or omitted).
 * @property bool $DaySunday If the enrollment occurs on Sunday(s).
 * @property bool $DayMonday If the enrollment occurs on Monday(s).
 * @property bool $DayTuesday If the enrollment occurs on Tuesday(s).
 * @property bool $DayWednesday If the enrollment occurs on Wednesday(s).
 * @property bool $DayThursday If the enrollment occurs on Thursday(s).
 * @property bool $DayFriday If the enrollment occurs on Friday(s).
 * @property bool $DaySaturday If the enrollment occurs on Saturday(s).
 * @property int $StaffId The staff member teaching the enrollment.
 * @property int $StaffPayRate The staff pay rate. Must be between 1-21.
 * @property int $ResourceId The room where the enrollment is taking place.
 * @property int $MaxCapacity How many people can attend.
 * @property int $WebCapacity How many people can signup online.  Default: **0**
 * @property int $WaitlistCapacity How many people can waitlist.  Default:**0**
 * @property string $BookingStatus One of: PaymentRequired, BookAndPayLater, Free
 * @property bool $AllowOpenEnrollment Allow clients to choose which sessions they’d like to sign up for.  Default: **false**
 * @property bool $AllowDateForwardEnrollment Allow booking after the enrollment has started.  Default: **false**
 * @property int[] $PricingOptionsProductIds Pricing Options for this schedule
 * @property bool $ShowToPublic Allow clients to see this schedule  Default: **true**
 *
 */
class AddClassEnrollmentScheduleRequest extends BaseModel
{
    const DISCRIMINATOR = null;

    /**
      * The original name of the model.
      *
      * @var string
      */
    protected static string $swaggerModelName = 'AddClassEnrollmentScheduleRequest';

    /**
      * Array of property to type mappings. Used for (de)serialization
      *
      * @var string[]
      */
    protected static array $swaggerTypes = [
        'ClassDescriptionId' => 'int',
        'LocationId' => 'int',
        'StartDate' => '\DateTime',
        'EndDate' => '\DateTime',
        'StartTime' => '\DateTime',
        'EndTime' => '\DateTime',
        'DaySunday' => 'bool',
        'DayMonday' => 'bool',
        'DayTuesday' => 'bool',
        'DayWednesday' => 'bool',
        'DayThursday' => 'bool',
        'DayFriday' => 'bool',
        'DaySaturday' => 'bool',
        'StaffId' => 'int',
        'StaffPayRate' => 'int',
        'ResourceId' => 'int',
        'MaxCapacity' => 'int',
        'WebCapacity' => 'int',
        'WaitlistCapacity' => 'int',
        'BookingStatus' => 'string',
        'AllowOpenEnrollment' => 'bool',
        'AllowDateForwardEnrollment' => 'bool',
        'PricingOptionsProductIds' => 'int[]',
        'ShowToPublic' => 'bool'
    ];

    /**
      * Array of property to format mappings. Used for (de)serialization
      *
      * @var array<string, string|null>
      */
    protected static array $swaggerFormats = [
        'ClassDescriptionId' => 'int32',
        'LocationId' => 'int32',
        'StartDate' => 'date-time',
        'EndDate' => 'date-time',
        'StartTime' => 'date-time',
        'EndTime' => 'date-time',
        'DaySunday' => null,
        'DayMonday' => null,
        'DayTuesday' => null,
        'DayWednesday' => null,
        'DayThursday' => null,
        'DayFriday' => null,
        'DaySaturday' => null,
        'StaffId' => 'int64',
        'StaffPayRate' => 'int32',
        'ResourceId' => 'int32',
        'MaxCapacity' => 'int32',
        'WebCapacity' => 'int32',
        'WaitlistCapacity' => 'int32',
        'BookingStatus' => null,
        'AllowOpenEnrollment' => null,
        'AllowDateForwardEnrollment' => null,
        'PricingOptionsProductIds' => 'int32',
        'ShowToPublic' => null
    ];


    /**
     * Array of attributes where the key is the local name,
     * and the value is the original name
     *
     * @var string[]
     */
    protected static array $attributeMap = [
        'ClassDescriptionId' => 'ClassDescriptionId',
        'LocationId' => 'LocationId',
        'StartDate' => 'StartDate',
        'EndDate' => 'EndDate',
        'StartTime' => 'StartTime',
        'EndTime' => 'EndTime',
        'DaySunday' => 'DaySunday',
        'DayMonday' => 'DayMonday',
        'DayTuesday' => 'DayTuesday',
        'DayWednesday' => 'DayWednesday',
        'DayThursday' => 'DayThursday',
        'DayFriday' => 'DayFriday',
        'DaySaturday' => 'DaySaturday',
        'StaffId' => 'StaffId',
        'StaffPayRate' => 'StaffPayRate',
        'ResourceId' => 'ResourceId',
        'MaxCapacity' => 'MaxCapacity',
        'WebCapacity' => 'WebCapacity',
        'WaitlistCapacity' => 'WaitlistCapacity',
        'BookingStatus' => 'BookingStatus',
        'AllowOpenEnrollment' => 'AllowOpenEnrollment',
        'AllowDateForwardEnrollment' => 'AllowDateForwardEnrollment',
        'PricingOptionsProductIds' => 'PricingOptionsProductIds',
        'ShowToPublic' => 'ShowToPublic'
    ];

    /**
     * Array of attributes to setter functions (for deserialization of responses)
     *
     * @var string[]
     */
    protected static array $setters = [
        'ClassDescriptionId' => 'setClassDescriptionId',
        'LocationId' => 'setLocationId',
        'StartDate' => 'setStartDate',
        'EndDate' => 'setEndDate',
        'StartTime' => 'setStartTime',
        'EndTime' => 'setEndTime',
        'DaySunday' => 'setDaySunday',
        'DayMonday' => 'setDayMonday',
        'DayTuesday' => 'setDayTuesday',
        'DayWednesday' => 'setDayWednesday',
        'DayThursday' => 'setDayThursday',
        'DayFriday' => 'setDayFriday',
        'DaySaturday' => 'setDaySaturday',
        'StaffId' => 'setStaffId',
        'StaffPayRate' => 'setStaffPayRate',
        'ResourceId' => 'setResourceId',
        'MaxCapacity' => 'setMaxCapacity',
        'WebCapacity' => 'setWebCapacity',
        'WaitlistCapacity' => 'setWaitlistCapacity',
        'BookingStatus' => 'setBookingStatus',
        'AllowOpenEnrollment' => 'setAllowOpenEnrollment',
        'AllowDateForwardEnrollment' => 'setAllowDateForwardEnrollment',
        'PricingOptionsProductIds' => 'setPricingOptionsProductIds',
        'ShowToPublic' => 'setShowToPublic'
    ];

    /**
     * Array of attributes to getter functions (for serialization of requests)
     *
     * @var string[]
     */
    protected static array $getters = [
        'ClassDescriptionId' => 'getClassDescriptionId',
        'LocationId' => 'getLocationId',
        'StartDate' => 'getStartDate',
        'EndDate' => 'getEndDate',
        'StartTime' => 'getStartTime',
        'EndTime' => 'getEndTime',
        'DaySunday' => 'getDaySunday',
        'DayMonday' => 'getDayMonday',
        'DayTuesday' => 'getDayTuesday',
        'DayWednesday' => 'getDayWednesday',
        'DayThursday' => 'getDayThursday',
        'DayFriday' => 'getDayFriday',
        'DaySaturday' => 'getDaySaturday',
        'StaffId' => 'getStaffId',
        'StaffPayRate' => 'getStaffPayRate',
        'ResourceId' => 'getResourceId',
        'MaxCapacity' => 'getMaxCapacity',
        'WebCapacity' => 'getWebCapacity',
        'WaitlistCapacity' => 'getWaitlistCapacity',
        'BookingStatus' => 'getBookingStatus',
        'AllowOpenEnrollment' => 'getAllowOpenEnrollment',
        'AllowDateForwardEnrollment' => 'getAllowDateForwardEnrollment',
        'PricingOptionsProductIds' => 'getPricingOptionsProductIds',
        'ShowToPublic' => 'getShowToPublic'
    ];


    

    

    /**
     * Constructor
     *
     * @param mixed[] $data Associated array of property values
     *                      initializing the model
     */
    public function __construct(?array $data = null)
    {
        $this->container['ClassDescriptionId'] = isset($data['ClassDescriptionId']) ? $data['ClassDescriptionId'] : null;
        $this->container['LocationId'] = isset($data['LocationId']) ? $data['LocationId'] : null;
        $this->container['StartDate'] = isset($data['StartDate']) ? $data['StartDate'] : null;
        $this->container['EndDate'] = isset($data['EndDate']) ? $data['EndDate'] : null;
        $this->container['StartTime'] = isset($data['StartTime']) ? $data['StartTime'] : null;
        $this->container['EndTime'] = isset($data['EndTime']) ? $data['EndTime'] : null;
        $this->container['DaySunday'] = isset($data['DaySunday']) ? $data['DaySunday'] : null;
        $this->container['DayMonday'] = isset($data['DayMonday']) ? $data['DayMonday'] : null;
        $this->container['DayTuesday'] = isset($data['DayTuesday']) ? $data['DayTuesday'] : null;
        $this->container['DayWednesday'] = isset($data['DayWednesday']) ? $data['DayWednesday'] : null;
        $this->container['DayThursday'] = isset($data['DayThursday']) ? $data['DayThursday'] : null;
        $this->container['DayFriday'] = isset($data['DayFriday']) ? $data['DayFriday'] : null;
        $this->container['DaySaturday'] = isset($data['DaySaturday']) ? $data['DaySaturday'] : null;
        $this->container['StaffId'] = isset($data['StaffId']) ? $data['StaffId'] : null;
        $this->container['StaffPayRate'] = isset($data['StaffPayRate']) ? $data['StaffPayRate'] : null;
        $this->container['ResourceId'] = isset($data['ResourceId']) ? $data['ResourceId'] : null;
        $this->container['MaxCapacity'] = isset($data['MaxCapacity']) ? $data['MaxCapacity'] : null;
        $this->container['WebCapacity'] = isset($data['WebCapacity']) ? $data['WebCapacity'] : null;
        $this->container['WaitlistCapacity'] = isset($data['WaitlistCapacity']) ? $data['WaitlistCapacity'] : null;
        $this->container['BookingStatus'] = isset($data['BookingStatus']) ? $data['BookingStatus'] : null;
        $this->container['AllowOpenEnrollment'] = isset($data['AllowOpenEnrollment']) ? $data['AllowOpenEnrollment'] : null;
        $this->container['AllowDateForwardEnrollment'] = isset($data['AllowDateForwardEnrollment']) ? $data['AllowDateForwardEnrollment'] : null;
        $this->container['PricingOptionsProductIds'] = isset($data['PricingOptionsProductIds']) ? $data['PricingOptionsProductIds'] : null;
        $this->container['ShowToPublic'] = isset($data['ShowToPublic']) ? $data['ShowToPublic'] : null;
    }

    /**
     * Show all the invalid properties with reasons.
     *
     * @return string[] invalid properties with reasons
     */
    public function listInvalidProperties(): array
    {
        $invalidProperties = parent::listInvalidProperties();

        return $invalidProperties;
    }


    /**
     * Gets ClassDescriptionId
     *
     * @return int
     */
    public function getClassDescriptionId(): int|null
    {
        return $this->container['ClassDescriptionId'];
    }

    /**
     * Sets ClassDescriptionId
     *
     * @param int $ClassDescriptionId The Id of the class/enrollment description. This can be found in GetClassDescriptions.
     *
     * @return $this
     */


    public function setClassDescriptionId(int $ClassDescriptionId): static
    {
        $this->container['ClassDescriptionId'] = $ClassDescriptionId;

        return $this;
    }

    /**
     * Gets LocationId
     *
     * @return int
     */
    public function getLocationId(): int|null
    {
        return $this->container['LocationId'];
    }

    /**
     * Sets LocationId
     *
     * @param int $LocationId The Location Id of the enrollment schedule.
     *
     * @return $this
     */


    public function setLocationId(int $LocationId): static
    {
        $this->container['LocationId'] = $LocationId;

        return $this;
    }

    /**
     * Gets StartDate
     *
     * @return \DateTime
     */
    public function getStartDate(): \DateTime|null
    {
        return $this->container['StartDate'];
    }

    /**
     * Sets StartDate
     *
     * @param \DateTime $StartDate The start date of the enrollment schedule.
     *
     * @return $this
     */


    public function setStartDate(\DateTime $StartDate): static
    {
        $this->container['StartDate'] = $StartDate;

        return $this;
    }

    /**
     * Gets EndDate
     *
     * @return \DateTime
     */
    public function getEndDate(): \DateTime|null
    {
        return $this->container['EndDate'];
    }

    /**
     * Sets EndDate
     *
     * @param \DateTime $EndDate The end date of the enrollment schedule.
     *
     * @return $this
     */


    public function setEndDate(\DateTime $EndDate): static
    {
        $this->container['EndDate'] = $EndDate;

        return $this;
    }

    /**
     * Gets StartTime
     *
     * @return \DateTime
     */
    public function getStartTime(): \DateTime|null
    {
        return $this->container['StartTime'];
    }

    /**
     * Sets StartTime
     *
     * @param \DateTime $StartTime Enrollment start time (use null or omit for TBD).
     *
     * @return $this
     */


    public function setStartTime(\DateTime $StartTime): static
    {
        $this->container['StartTime'] = $StartTime;

        return $this;
    }

    /**
     * Gets EndTime
     *
     * @return \DateTime
     */
    public function getEndTime(): \DateTime|null
    {
        return $this->container['EndTime'];
    }

    /**
     * Sets EndTime
     *
     * @param \DateTime $EndTime Enrollment end time (ignored if StartTime is null or omitted).
     *
     * @return $this
     */


    public function setEndTime(\DateTime $EndTime): static
    {
        $this->container['EndTime'] = $EndTime;

        return $this;
    }

    /**
     * Gets DaySunday
     *
     * @return bool
     */
    public function getDaySunday(): bool|null
    {
        return $this->container['DaySunday'];
    }

    /**
     * Sets DaySunday
     *
     * @param bool $DaySunday If the enrollment occurs on Sunday(s).
     *
     * @return $this
     */


    public function setDaySunday(bool $DaySunday): static
    {
        $this->container['DaySunday'] = $DaySunday;

        return $this;
    }

    /**
     * Gets DayMonday
     *
     * @return bool
     */
    public function getDayMonday(): bool|null
    {
        return $this->container['DayMonday'];
    }

    /**
     * Sets DayMonday
     *
     * @param bool $DayMonday If the enrollment occurs on Monday(s).
     *
     * @return $this
     */


    public function setDayMonday(bool $DayMonday): static
    {
        $this->container['DayMonday'] = $DayMonday;

        return $this;
    }

    /**
     * Gets DayTuesday
     *
     * @return bool
     */
    public function getDayTuesday(): bool|null
    {
        return $this->container['DayTuesday'];
    }

    /**
     * Sets DayTuesday
     *
     * @param bool $DayTuesday If the enrollment occurs on Tuesday(s).
     *
     * @return $this
     */


    public function setDayTuesday(bool $DayTuesday): static
    {
        $this->container['DayTuesday'] = $DayTuesday;

        return $this;
    }

    /**
     * Gets DayWednesday
     *
     * @return bool
     */
    public function getDayWednesday(): bool|null
    {
        return $this->container['DayWednesday'];
    }

    /**
     * Sets DayWednesday
     *
     * @param bool $DayWednesday If the enrollment occurs on Wednesday(s).
     *
     * @return $this
     */


    public function setDayWednesday(bool $DayWednesday): static
    {
        $this->container['DayWednesday'] = $DayWednesday;

        return $this;
    }

    /**
     * Gets DayThursday
     *
     * @return bool
     */
    public function getDayThursday(): bool|null
    {
        return $this->container['DayThursday'];
    }

    /**
     * Sets DayThursday
     *
     * @param bool $DayThursday If the enrollment occurs on Thursday(s).
     *
     * @return $this
     */


    public function setDayThursday(bool $DayThursday): static
    {
        $this->container['DayThursday'] = $DayThursday;

        return $this;
    }

    /**
     * Gets DayFriday
     *
     * @return bool
     */
    public function getDayFriday(): bool|null
    {
        return $this->container['DayFriday'];
    }

    /**
     * Sets DayFriday
     *
     * @param bool $DayFriday If the enrollment occurs on Friday(s).
     *
     * @return $this
     */


    public function setDayFriday(bool $DayFriday): static
    {
        $this->container['DayFriday'] = $DayFriday;

        return $this;
    }

    /**
     * Gets DaySaturday
     *
     * @return bool
     */
    public function getDaySaturday(): bool|null
    {
        return $this->container['DaySaturday'];
    }

    /**
     * Sets DaySaturday
     *
     * @param bool $DaySaturday If the enrollment occurs on Saturday(s).
     *
     * @return $this
     */


    public function setDaySaturday(bool $DaySaturday): static
    {
        $this->container['DaySaturday'] = $DaySaturday;

        return $this;
    }

    /**
     * Gets StaffId
     *
     * @return int
     */
    public function getStaffId(): int|null
    {
        return $this->container['StaffId'];
    }

    /**
     * Sets StaffId
     *
     * @param int $StaffId The staff member teaching the enrollment.
     *
     * @return $this
     */


    public function setStaffId(int $StaffId): static
    {
        $this->container['StaffId'] = $StaffId;

        return $this;
    }

    /**
     * Gets StaffPayRate
     *
     * @return int
     */
    public function getStaffPayRate(): int|null
    {
        return $this->container['StaffPayRate'];
    }

    /**
     * Sets StaffPayRate
     *
     * @param int $StaffPayRate The staff pay rate. Must be between 1-21.
     *
     * @return $this
     */


    public function setStaffPayRate(int $StaffPayRate): static
    {
        $this->container['StaffPayRate'] = $StaffPayRate;

        return $this;
    }

    /**
     * Gets ResourceId
     *
     * @return int
     */
    public function getResourceId(): int|null
    {
        return $this->container['ResourceId'];
    }

    /**
     * Sets ResourceId
     *
     * @param int $ResourceId The room where the enrollment is taking place.
     *
     * @return $this
     */


    public function setResourceId(int $ResourceId): static
    {
        $this->container['ResourceId'] = $ResourceId;

        return $this;
    }

    /**
     * Gets MaxCapacity
     *
     * @return int
     */
    public function getMaxCapacity(): int|null
    {
        return $this->container['MaxCapacity'];
    }

    /**
     * Sets MaxCapacity
     *
     * @param int $MaxCapacity How many people can attend.
     *
     * @return $this
     */


    public function setMaxCapacity(int $MaxCapacity): static
    {
        $this->container['MaxCapacity'] = $MaxCapacity;

        return $this;
    }

    /**
     * Gets WebCapacity
     *
     * @return int
     */
    public function getWebCapacity(): int|null
    {
        return $this->container['WebCapacity'];
    }

    /**
     * Sets WebCapacity
     *
     * @param int $WebCapacity How many people can signup online.  Default: **0**
     *
     * @return $this
     */


    public function setWebCapacity(int $WebCapacity): static
    {
        $this->container['WebCapacity'] = $WebCapacity;

        return $this;
    }

    /**
     * Gets WaitlistCapacity
     *
     * @return int
     */
    public function getWaitlistCapacity(): int|null
    {
        return $this->container['WaitlistCapacity'];
    }

    /**
     * Sets WaitlistCapacity
     *
     * @param int $WaitlistCapacity How many people can waitlist.  Default:**0**
     *
     * @return $this
     */


    public function setWaitlistCapacity(int $WaitlistCapacity): static
    {
        $this->container['WaitlistCapacity'] = $WaitlistCapacity;

        return $this;
    }

    /**
     * Gets BookingStatus
     *
     * @return string
     */
    public function getBookingStatus(): string|null
    {
        return $this->container['BookingStatus'];
    }

    /**
     * Sets BookingStatus
     *
     * @param string $BookingStatus One of: PaymentRequired, BookAndPayLater, Free
     *
     * @return $this
     */


    public function setBookingStatus(string $BookingStatus): static
    {
        $this->container['BookingStatus'] = $BookingStatus;

        return $this;
    }

    /**
     * Gets AllowOpenEnrollment
     *
     * @return bool
     */
    public function getAllowOpenEnrollment(): bool|null
    {
        return $this->container['AllowOpenEnrollment'];
    }

    /**
     * Sets AllowOpenEnrollment
     *
     * @param bool $AllowOpenEnrollment Allow clients to choose which sessions they’d like to sign up for.  Default: **false**
     *
     * @return $this
     */


    public function setAllowOpenEnrollment(bool $AllowOpenEnrollment): static
    {
        $this->container['AllowOpenEnrollment'] = $AllowOpenEnrollment;

        return $this;
    }

    /**
     * Gets AllowDateForwardEnrollment
     *
     * @return bool
     */
    public function getAllowDateForwardEnrollment(): bool|null
    {
        return $this->container['AllowDateForwardEnrollment'];
    }

    /**
     * Sets AllowDateForwardEnrollment
     *
     * @param bool $AllowDateForwardEnrollment Allow booking after the enrollment has started.  Default: **false**
     *
     * @return $this
     */


    public function setAllowDateForwardEnrollment(bool $AllowDateForwardEnrollment): static
    {
        $this->container['AllowDateForwardEnrollment'] = $AllowDateForwardEnrollment;

        return $this;
    }

    /**
     * Gets PricingOptionsProductIds
     *
     * @return int[]
     */
    public function getPricingOptionsProductIds(): array|null
    {
        return $this->container['PricingOptionsProductIds'];
    }

    /**
     * Sets PricingOptionsProductIds
     *
     * @param int[] $PricingOptionsProductIds Pricing Options for this schedule
     *
     * @return $this
     */


    public function setPricingOptionsProductIds(array $PricingOptionsProductIds): static
    {
        $this->container['PricingOptionsProductIds'] = $PricingOptionsProductIds;

        return $this;
    }

    /**
     * Gets ShowToPublic
     *
     * @return bool
     */
    public function getShowToPublic(): bool|null
    {
        return $this->container['ShowToPublic'];
    }

    /**
     * Sets ShowToPublic
     *
     * @param bool $ShowToPublic Allow clients to see this schedule  Default: **true**
     *
     * @return $this
     */


    public function setShowToPublic(bool $ShowToPublic): static
    {
        $this->container['ShowToPublic'] = $ShowToPublic;

        return $this;
    }
    
}


