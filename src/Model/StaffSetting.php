<?php
/**
 * StaffSetting
 *
 * PHP version 5
 *
 * @category Class
 * @package  Nlocascio\Mindbody
 * @author   Swagger Codegen team
 * @link     https://github.com/swagger-api/swagger-codegen
 */

/**
 * MINDBODY Public API
 *
 * No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)
 *
 * OpenAPI spec version: v6
 * 
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 * Swagger Codegen version: 2.4.40-SNAPSHOT
 */

/**
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen
 * Do not edit the class manually.
 */

namespace Nlocascio\Mindbody\Model;

/**
 * StaffSetting Class Doc Comment
 *
 * @category Class
 * @description contains the information about the staff settings.
 * @package  Nlocascio\Mindbody
 * @author   Swagger Codegen team
 * @link     https://github.com/swagger-api/swagger-codegen
 * @property bool $UseStaffNicknames When `true`, `DisplayName` of the staff will be displayed.  When `false`, `DisplayName` will be displayed as null.
 * @property bool $ShowStaffLastNamesOnSchedules When `true`, indicates that the Name contains both the `FirstName` and `LastName` of the staff.  When `false`, indicates that the Name contains only the `FirstName` of the staff.
 *
 */
class StaffSetting extends BaseModel
{
    const DISCRIMINATOR = null;

    /**
      * The original name of the model.
      *
      * @var string
      */
    protected static $swaggerModelName = 'StaffSetting';

    /**
      * Array of property to type mappings. Used for (de)serialization
      *
      * @var string[]
      */
    protected static $swaggerTypes = [
        'UseStaffNicknames' => 'bool',
        'ShowStaffLastNamesOnSchedules' => 'bool'
    ];

    /**
      * Array of property to format mappings. Used for (de)serialization
      *
      * @var array<string, string|null>
      */
    protected static $swaggerFormats = [
        'UseStaffNicknames' => null,
        'ShowStaffLastNamesOnSchedules' => null
    ];


    /**
     * Array of attributes where the key is the local name,
     * and the value is the original name
     *
     * @var string[]
     */
    protected static $attributeMap = [
        'UseStaffNicknames' => 'UseStaffNicknames',
        'ShowStaffLastNamesOnSchedules' => 'ShowStaffLastNamesOnSchedules'
    ];

    /**
     * Array of attributes to setter functions (for deserialization of responses)
     *
     * @var string[]
     */
    protected static $setters = [
        'UseStaffNicknames' => 'setUseStaffNicknames',
        'ShowStaffLastNamesOnSchedules' => 'setShowStaffLastNamesOnSchedules'
    ];

    /**
     * Array of attributes to getter functions (for serialization of requests)
     *
     * @var string[]
     */
    protected static $getters = [
        'UseStaffNicknames' => 'getUseStaffNicknames',
        'ShowStaffLastNamesOnSchedules' => 'getShowStaffLastNamesOnSchedules'
    ];


    

    

    /**
     * Constructor
     *
     * @param mixed[] $data Associated array of property values
     *                      initializing the model
     */
    public function __construct(array $data = null)
    {
        $this->container['UseStaffNicknames'] = isset($data['UseStaffNicknames']) ? $data['UseStaffNicknames'] : null;
        $this->container['ShowStaffLastNamesOnSchedules'] = isset($data['ShowStaffLastNamesOnSchedules']) ? $data['ShowStaffLastNamesOnSchedules'] : null;
    }

    /**
     * Show all the invalid properties with reasons.
     *
     * @return string[] invalid properties with reasons
     */
    public function listInvalidProperties(): array
    {
        $invalidProperties = parent::listInvalidProperties();

        return $invalidProperties;
    }


    /**
     * Gets UseStaffNicknames
     *
     * @return bool
     */
    public function getUseStaffNicknames()
    {
        return $this->container['UseStaffNicknames'];
    }

    /**
     * Sets UseStaffNicknames
     *
     * @param bool $UseStaffNicknames When `true`, `DisplayName` of the staff will be displayed.  When `false`, `DisplayName` will be displayed as null.
     *
     * @return $this
     */
    public function setUseStaffNicknames($UseStaffNicknames): self
    {
        $this->container['UseStaffNicknames'] = $UseStaffNicknames;

        return $this;
    }

    /**
     * Gets ShowStaffLastNamesOnSchedules
     *
     * @return bool
     */
    public function getShowStaffLastNamesOnSchedules()
    {
        return $this->container['ShowStaffLastNamesOnSchedules'];
    }

    /**
     * Sets ShowStaffLastNamesOnSchedules
     *
     * @param bool $ShowStaffLastNamesOnSchedules When `true`, indicates that the Name contains both the `FirstName` and `LastName` of the staff.  When `false`, indicates that the Name contains only the `FirstName` of the staff.
     *
     * @return $this
     */
    public function setShowStaffLastNamesOnSchedules($ShowStaffLastNamesOnSchedules): self
    {
        $this->container['ShowStaffLastNamesOnSchedules'] = $ShowStaffLastNamesOnSchedules;

        return $this;
    }
    
}


